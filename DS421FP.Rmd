...
#Story Map with Conductivity and Temperature Plot
...
# Define Kalanianaʻole Hwy route as LINESTRING
kalanianaole_route <- st_sfc(
  st_linestring(matrix(c(
    -157.779658, 21.277743,  # Start point (lng, lat)
    -157.724882, 21.285436   # End point (lng, lat)
  ), ncol = 2, byrow = TRUE)),
  crs = 4326
) %>%
  st_sf(name = "Kalanianaʻole Hwy Segment", geometry = .)
  ...
  story_map(
    ...
    sections = list(
      ...
      kanewai_marker = story_section(
        ...
        content = list(
          p("Located at 5975 HI-72, Kānewai Spring: An important traditional freshwater source for the local community.On June 24 peak conductivity data was collected. As shown on the chart below, Specific Conductance was recorded at 697.7079µS/cm which is the highest recorded conductivity reading within this current data set"),
      ...
      kalauhaihai_marker = story_section(
        ...
        content = list(
          p("Kalauhaʻihaʻi: a spring-fed loko iʻa in Maunalua Bay with ongoing restoration efforts.  "),
      ...
      kanewai_isochrone = story_section(
        ...
        content = list(
          p("Within a 20-minute drive of Kānewai Spring are key East Honolulu landmarks and communities.")
        )
      ),
      kalanianaole_highway = story_section(
  title = "Kalanianaʻole Highway Segment",
  content = list(
    p("This segment of Kalanianaʻole Hwy highlights a key corridor between Kānewai Spring and the southeastern coastline.")
  )
)
...
server <- function(input, output, session) {
  ...
  mapboxgl_proxy("spring_map") |>
    ...
    clear_layer("kanewai_layer") |>
    clear_layer("kalauhaihai_layer") |>
    clear_layer("kanewai_route_line") |>
    add_source("kalanianaole_route", data = kalanianaole_route) |>
    add_line_layer(
      id = "kalanianaole_route_line",
      source = "kalanianaole_route",
      line_color = "#E69F00",
      line_width = 4
    ) |>
    fit_bounds(
