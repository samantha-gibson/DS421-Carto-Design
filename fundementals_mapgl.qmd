...
hi_median_income <- hi_median_income |>
  mutate(
    poverty_flag = ifelse(estimate < 30000, "Below Poverty Line", "Above Poverty Line")
  )
```

```{r}
poverty_colors <- c("Below Poverty Line" = "#d73027", "Above Poverty Line" = "#1a9850")
...
mapboxgl(bounds = hi_median_income) |>
  add_fill_layer(
    ...
    fill_color = match_expr(
      column = "poverty_flag",
      values = names(poverty_colors),
      stops = unname(poverty_colors)
    ),
    fill_opacity = 0.6
  ) |>
  add_legend(
...
```{r}# Example: Median income in California}
ca_income <- get_acs(
  geography = "tract",
...
```{r}
# Example: Median age in New Jersey
...
mapboxgl(bounds = nj_age) |>
  add_fill_layer(
    id = "nj_scale",
    source = nj_age,
    fill_color = nj_scale$expression,
    fill_opacity = 0.5
  ) |>
  add_legend(
    "Median Age in New Jersey",
    values = get_legend_labels(nj_scale, digits = 0, prefix = "age"),
    colors = get_legend_colors(nj_scale),
    type = "continuous"
  )
