...
```{r}
ggplot(data = wildfire_counts, aes(y = number_of_fires, x = ACQ_DATE)) +
...
```{r}
ggplot(data = wildfire_shp_data, aes(x= ACQ_DATE, fill = CONFIDENCE)) +
  geom_bar()
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1753145203282.kml",
...
landsat_df <- read.csv("LANDSAT_USA_contiguous_and_Hawaii_7d.csv")

# TRANSFORM DATAFRAME INTO SF OBJECT 
landsat_df <- st_as_sf(landsat_df, coords = c("longitude", "latitude"), crs = 4326)
